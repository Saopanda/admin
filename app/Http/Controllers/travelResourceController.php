...
class travelResourceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $resource = travelresource::
            ...
            ->select('id','classid','name','site','des','price')
            ->get();
        return view('travel.resource.list',['Tclass'=>$Tclass,'resource'=>$resource,'classid'=>$classid]);
    }
    ...
    public function create()
    {
        $Tclass = travelclass::with([
            ...
            ->select('id','name','classid','icon')
            ->get();
        $a = Storage::get('123.txt');
        echo asset('123.txt');
        dd($a);
        return view('travel.resource.add',['Tclass'=>$Tclass]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $file = $request->hasFile($request->imgs[0]);
        if($file){
            //  进行上传图片动作
...
//                $img[$k] =
    ...
    public function edit($id)
    {
        ...
        $res->lxinfo = travelclass::where('classid',$res->classid)
            ->get();
        return view('travel.resource.edit',['res'=>$res]);
    }
