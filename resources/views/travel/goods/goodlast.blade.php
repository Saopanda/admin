            ...
            <div class="fl left">
                ...
                <ol>
                    @foreach($res as $k=>$v)
                        ...
                        <li>{{$v->resource_info->lxinfo->name}}：{{$v->resource_info->name}} <span style="float: right">￥{{$v->price}}</span></li>
                        @else
                        <li>资源失效 {{$v->price}}</li>
                        @endif
                    @endforeach
                ...
                <p>
                    ...
                    <span> 指导价: <input type="number" min="0" name="price_sel" @if($goods->price_sel == 0) placeholder="请输入指导价" @else value="{{$goods->price_sel}}" @endif></span>
                </p>
            </div>
            <div class="fl right">
                <h3>酒店住宿安排</h3>
